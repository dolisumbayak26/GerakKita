...
export default function DriverDashboard() {
    ...
    const loadDriverBus = async () => {
        // Strict guard: if no user ID, do nothing (prevents error on logout)
        if (!user?.id) return;
        ...
        try {
            ...
            if (error) {
                // Ignore specific errors that might happen during auth transitions
            ...
            // Only alert if it's a real network/data error and user is still logged in
            if (user?.id) {
                console.error('Error loading driver bus:', error);
                // Suppress "invalid input syntax for type uuid" which happens on logout
                if (!error.message?.includes('invalid input syntax')) {
                    Alert.alert('Error', 'Gagal memuat info bus Anda');
                }
            }
        } finally {
            if (user?.id) { // Only update state if user is still present
                setLoading(false);
            }
        }
    };
                    ...
                    <View style={[styles.card, { backgroundColor: theme.card }]}>
                        <View style={styles.emptyState}>
                            ...
                            <Text style={[styles.emptyText, { color: theme.textSecondary }]}>
                                Belum ada bus yang dipilih
                            </Text>
                            <TouchableOpacity
                                style={[styles.selectBusButton, { backgroundColor: theme.primary }]}
                                onPress={() => router.push('/portal-driver/select-bus')}
                            >
                                <Text style={styles.selectBusText}>Pilih Bus Sekarang</Text>
                            </TouchableOpacity>
                        </View>
                    </View>
                )}
